...
class GenerationTask:
    ...
    def run(self) -> bool:
        ...
        if self.tec == 'cgra' and self.gen_mode == 'grammar':
            return self._run_cgra_grammar()
        # Other modes can be added here
        logger.error(f"Unsupported generation mode: {self.tec} with {self.gen_mode}")
        return False
    ...
    def _save_all_files(self, graph, tec_name, base_folder, arch_size, index, **kwargs):
        ...
        if tec_name == "CGRA":
            ...
            # O 'II' agora vem antes no caminho do diretório.
        ...
        # A lógica para gerar apenas o .dot pode ser adicionada ao Visualizer se necessário
